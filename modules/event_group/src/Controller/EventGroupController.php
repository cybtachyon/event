...
/**
 * Returns responses for 'event_group' GroupContent routes.
 */
class EventGroupController extends GroupContentController {
  ...
  /**
   * Constructs a new EventGroupController.
   *
  ...
  public function addPage(GroupInterface $group, $create_mode = FALSE) {
    ...
    $storage_handler = $this->entityTypeManager->getStorage('event_type');
    foreach ($this->addPageBundles($group, $create_mode) as $plugin_id => $bundle_name) {
      if (!empty($build['#bundles'][$bundle_name])) {
        ...
        $t_args = ['%event_type' => $bundle_label];
        $description = $create_mode
          ? $this->t('Add new content of type %event_type to the group.', $t_args)
          : $this->t('Add existing content of type %event_type to the group.', $t_args);
